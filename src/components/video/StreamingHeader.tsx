...
export const StreamingHeader: React.FC<Props> = (props) => {
  ...
  const getSettingLink = () => {
    if (UserHelper.checkAccess(Permissions.contentApi.content.edit) || UserHelper.checkAccess(Permissions.contentApi.streamingServices.edit)) {
      ...
      return (
        <li className="nav-item"><a href={`${EnvironmentHelper.Common.B1AdminRoot}/login?jwt=${jwt}&churchId=${churchId}&returnUrl=/`} className="nav-link" data-testid="stream-admin-link">{Locale.label("video.adminDashboard")}</a></li>
      );
    }
  };
